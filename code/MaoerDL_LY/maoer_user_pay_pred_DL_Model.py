...
def history_feature_mask(user_history_feature_index, data_matrix_user_history, max_history_len):
    ...
    return processed_feature_data, mask_history_feature_matrix
    ...
    for user_id in train_or_val_or_test_list:
        ...
        for i in range(len(user_data)):
            if i != len(user_data) - 1:  # 除最后一行即所有历史记录，不包括目标记录
                if user_data.iloc[i, -1] == 0:  # 非付费
                    data_matrix_not_pay_QOE_continue.append([user_data.iloc[i, col] for col in user_history_QOE_continue_index])   # 用户历史非付费QOE连续特征
                    data_matrix_not_pay_QOE_discrete.append([user_data.iloc[i, col] for col in user_history_QOE_discrete_index])   # 用户历史非付费QOE离散特征
                    data_matrix_not_pay_CHONGHE_continue.append(
                        [user_data.iloc[i, col] for col in user_history_CHONGHE_continue_index])  # 用户历史非付费CHONGHE连续特征
                    data_matrix_not_pay_CHONGHE_discrete.append(
                        [user_data.iloc[i, col] for col in user_history_CHONGHE_discrete_index])  # 用户历史非付费CHONGHE离散特征
                    data_matrix_not_pay_FUFEI_continue.append(
                        [user_data.iloc[i, col] for col in user_history_FUFEI_continue_index])  # 用户历史非付费FUFEI连续特征
                    data_matrix_not_pay_FUFEI_discrete.append(
                        [user_data.iloc[i, col] for col in user_history_FUFEI_discrete_index])  # 用户历史非付费FUFEI离散特征
                elif user_data.iloc[i, -1] == 1:
                    data_matrix_not_pay_QOE_continue.append(
                        [user_data.iloc[i, col] for col in user_history_QOE_continue_index])  # 用户历史付费QOE连续特征
                    data_matrix_not_pay_QOE_discrete.append(
                        [user_data.iloc[i, col] for col in user_history_QOE_discrete_index])  # 用户历史付费QOE离散特征
                    data_matrix_not_pay_CHONGHE_continue.append(
                        [user_data.iloc[i, col] for col in user_history_CHONGHE_continue_index])  # 用户历史付费CHONGHE连续特征
                    data_matrix_not_pay_CHONGHE_discrete.append(
                        [user_data.iloc[i, col] for col in user_history_CHONGHE_discrete_index])  # 用户历史付费CHONGHE离散特征
                    data_matrix_not_pay_FUFEI_continue.append(
                        [user_data.iloc[i, col] for col in user_history_FUFEI_continue_index])  # 用户历史付费FUFEI连续特征
                    data_matrix_not_pay_FUFEI_discrete.append(
                        [user_data.iloc[i, col] for col in user_history_FUFEI_discrete_index])  # 用户历史付费FUFEI离散特征
            else:   # 目标记录
        ...
        data_matrix_not_pay_CHONGHE_discrete,data_matrix_not_pay_CHONGHE_discrete_mask = history_feature_mask(user_history_CHONGHE_discrete_index, data_matrix_not_pay_CHONGHE_continue,max_history_len)
        ...
        data_tensor_not_pay_CHONGHE_continue = torch.tensor(np.array(data_matrix_not_pay_CHONGHE_continue), dtype=torch.float32)
        data_tensor_not_pay_CHONGHE_discrete = torch.tensor(np.array(data_matrix_not_pay_CHONGHE_discrete), dtype=torch.float32)
        data_tensor_not_pay_QOE_continue = torch.tensor(np.array(data_matrix_not_pay_QOE_continue), dtype=torch.float32)
        data_tensor_not_pay_QOE_discrete = torch.tensor(np.array(data_matrix_not_pay_QOE_discrete), dtype=torch.float32)
        data_tensor_not_pay_FUFEI_continue = torch.tensor(np.array(data_matrix_not_pay_FUFEI_continue), dtype=torch.float32)
        data_tensor_not_pay_FUFEI_discrete = torch.tensor(np.array(data_matrix_not_pay_FUFEI_discrete), dtype=torch.float32)
        #  mask矩阵
        data_tensor_not_pay_QOE_continue_mask = torch.tensor(np.array(data_matrix_not_pay_QOE_continue_mask), dtype=torch.float32)
        data_tensor_not_pay_QOE_discrete_mask = torch.tensor(np.array(data_matrix_not_pay_QOE_discrete_mask), dtype=torch.float32)
        ...
        data_tensor_not_pay_CHONGHE_continue_mask = torch.tensor(np.array(data_matrix_not_pay_CHONGHE_continue_mask),dtype=torch.float32)
        data_tensor_not_pay_CHONGHE_discrete_mask = torch.tensor(np.array(data_matrix_not_pay_CHONGHE_discrete_mask),dtype=torch.float32)
        ...
        data_tensor_not_pay_FUFEI_continue_mask = torch.tensor(np.array(data_matrix_not_pay_FUFEI_continue_mask),dtype=torch.float32)
        data_tensor_not_pay_FUFEI_discrete_mask = torch.tensor(np.array(data_matrix_not_pay_FUFEI_discrete_mask),dtype=torch.float32)
        ...
        tensor_hash_value = {
            ...
            'user_info_continue': data_tensor_user_info_continue,
            'user_info_discrete': data_tensor_user_info_discrete,
...
def generate_user_feature_alignment_tensor(data_tensor_hash):
    ...
    not_pay_QOE_continue_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_QOE_continue')
    not_pay_QOE_discrete_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_QOE_discrete')
    ...
    not_pay_CHONGHE_continue_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_CHONGHE_continue')
    not_pay_CHONGHE_discrete_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_CHONGHE_discrete')
    ...
    not_pay_FUFEI_continue_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_FUFEI_continue')
    not_pay_FUFEI_discrete_batch_feature_tensor = generate_batch_feature(data_tensor_hash, 'not_pay_FUFEI_discrete')
    ...
    if 'user_info_continue' in data_tensor_hash:
        ...
        batch_feature_tensor_dict = {
            ...
            'user_info_continue': user_info_continue_batch_feature_tensor,
            'user_info_discrete': user_info_discrete_batch_feature_tensor,
...
# 非付费用户历史
class UserNotPayHistoryEmbedding(nn.Module):
    def __init__(self, continue_embedding_dim, discrete_embedding_dim, feature_category_num_dict, feature_column_dict):
        super(UserNotPayHistoryEmbedding, self).__init__()
        # 连续特征
        # 离散特征
        self.feature_category_num_dict = feature_category_num_dict
        # 离散embedding
        self.user_not_pay_history_QOE_discrete_embeddings = discrete_embedding(self.feature_category_num_dict,
                                                                feature_column_dict['history_QOE_discrete'],
                                                                discrete_embedding_dim)
        self.user_not_pay_history_CHONGHE_discrete_embeddings = discrete_embedding(self.feature_category_num_dict,
                                                                           feature_column_dict['history_CHONGHE_discrete'],
                                                                           discrete_embedding_dim)
        self.user_not_pay_history_FUFEI_discrete_embeddings = discrete_embedding(self.feature_category_num_dict,
                                                                           feature_column_dict['history_FUFEI_discrete'],
                                                                           discrete_embedding_dim)
        # MLP  连续embedding
        self.user_not_pay_history_QOE_continue_dense_layer = dense_layer(feature_category_num_dict['history_QOE_continue'].shape[2], continue_embedding_dim)
        self.user_not_pay_history_CHONGHE_continue_dense_layer = dense_layer(feature_category_num_dict['history_CHONGHE_continue'].shape[2], continue_embedding_dim)
        self.user_not_pay_history_FUFEI_continue_dense_layer = dense_layer(feature_category_num_dict['history_FUFEI_continue'].shape[2], continue_embedding_dim)

    def forward(self, batch_feature_tensor_dict):
        # user_history Embedding
        # user_history_continue_features_embedding (batch, 1, continue_feature_num, continue_embedding_dim)
        # user_history_discrete_features_embedding (batch, 1, discrete_feature_num, discrete_embedding_dim)
        # history中有三种：QOE/CHONGHE/FUFEI,将其分别转化为embedding然后合并
        user_history_not_pay_QOE_discrete_column_discrete_features_embedding = torch.stack(
            [embedding_layer(batch_feature_tensor_dict['not_pay_QOE_discrete'][:, :, i]) for i, embedding_layer in
             enumerate(self.user_not_pay_history_QOE_discrete_embeddings)], dim=-2)
        user_history_not_pay_QOE_continue_column_discrete_features_embedding = torch.stack(self.user_not_pay_history_QOE_continue_dense_layer(
            batch_feature_tensor_dict['not_pay_QOE_continue']), dim=-2)
        user_history_not_pay_QOE_vec = torch.cat(
            [user_history_not_pay_QOE_discrete_column_discrete_features_embedding, user_history_not_pay_QOE_continue_column_discrete_features_embedding], dim=2)  # 特征级合并
        user_history_not_pay_CHONGHE_discrete_column_discrete_features_embedding = torch.stack(
            [embedding_layer(batch_feature_tensor_dict['not_pay_CHONGHE_discrete'][:, :, i]) for i, embedding_layer in
             enumerate(self.user_not_pay_history_CHONGHE_discrete_embeddings)], dim=-2)
        user_history_not_pay_CHONGHE_continue_column_discrete_features_embedding = torch.stack(
            self.user_not_pay_history_CHONGHE_continue_dense_layer(
                batch_feature_tensor_dict['not_pay_CHONGHE_continue']), dim=-2)
        user_history_not_pay_CHONGHE_vec = torch.cat(
            [user_history_not_pay_CHONGHE_discrete_column_discrete_features_embedding,
             user_history_not_pay_CHONGHE_continue_column_discrete_features_embedding], dim=2)  # 特征级合并
        user_history_not_pay_FUFEI_discrete_column_discrete_features_embedding = torch.stack(
            [embedding_layer(batch_feature_tensor_dict['not_pay_FUFEI_discrete'][:, :, i]) for i, embedding_layer in
             enumerate(self.user_not_pay_history_FUFEI_discrete_embeddings)], dim=-2)
        user_history_not_pay_FUFEI_continue_column_discrete_features_embedding = torch.stack(
            self.user_not_pay_history_FUFEI_continue_dense_layer(
                batch_feature_tensor_dict['not_pay_FUFEI_continue']), dim=-2)
        user_history_not_pay_FUFEI_vec = torch.cat(
            [user_history_not_pay_FUFEI_discrete_column_discrete_features_embedding,
             user_history_not_pay_FUFEI_continue_column_discrete_features_embedding], dim=2)  # 特征级合并

        return user_history_not_pay_QOE_vec, user_history_not_pay_CHONGHE_vec, user_history_not_pay_FUFEI_vec
...
class HistoryDimScalingLayer(nn.Module):
    ...
    def forward(self, user_history_QOE_vec,  user_history_CHONGHE_vec, user_history_FUFEI_vec, mask=None):
        ...
        se_QOE_weight, se_user_history_QOE_vec = self.se_attention_QOE(multi_user_history_QOE_vec)
        se_CHONGHE_weight, se_user_history_CHONGHE_vec = self.se_attention_CHONGHE(multi_user_history_CHONGHE_vec)
        se_FUFEI_weight, se_user_history_FUFEI_vec = self.se_attention_FUFEI(multi_user_history_FUFEI_vec)
...
class History_Target_AttentionLayer(nn.Module):
    ...
    def forward(self, se_user_history_pay_QOE_vec, se_user_history_pay_CHONGHE_vec, se_user_history_pay_FUFEI_vec,se_user_history_not_pay_QOE_vec, se_user_history_not_pay_CHONGHE_vec, se_user_history_not_pay_FUFEI_vec, target_QOE_vec, target_CHONGHE_vec, target_FUFEI_vec, mask):
        ...
        user_history_not_pay_vec = torch.cat(se_user_history_not_pay_QOE_vec, se_user_history_not_pay_CHONGHE_vec, se_user_history_not_pay_FUFEI_vec, dim=1)
        target_vec = torch.cat(target_QOE_vec, target_CHONGHE_vec,target_FUFEI_vec, dim=2)
        # 对目标特征求对付费及非付费特征的偏好   (batch, 3，feature_dim)输出
        target_vec = target_vec.squeeze(1)  # 使用 squeeze 函数移除大小为 1 的维度
        target_history_pay_attention_weight, target_history_pay_attention_vec = self.target_history_pay_feature_pianhao_layer(target_vec, user_history_pay_vec, mask = None)
        target_history_not_pay_attention_weight, target_history_not_pay_attention_vec = self.target_history_not_pay_feature_pianhao_layer(
            target_vec, user_history_not_pay_vec, mask=None)

        return target_history_pay_attention_weight, target_history_not_pay_attention_weight, target_history_pay_attention_vec, target_history_not_pay_attention_vec

# (batch,600)经过网络变成200 +(batch,featuer_user*200)经过网络变成200 -> (batch,400)
# (batch,400) ->MLP ->(batch，1) ->sigmoid -> (batch,1)
...
class MatchingModel(nn.Module):
    def __init__(self, feature_category_num_dict, feature_column_dict, continue_embedding_dim,
        ...
        self.user_history_not_pay_embedding_layer = UserNotPayHistoryEmbedding(continue_embedding_dim, discrete_embedding_dim,feature_category_num_dict, feature_column_dict)
        ...
        # MLP
        final_dim = (len(feature_category_num_dict['user_info_continue'])+len(feature_category_num_dict['user_info_discrete'])+3)*200
        self.pay_vec_MLP_layer = dense_layer(final_dim, 1)
        self.not_pay_vec_MLP_layer = dense_layer(final_dim, 1)


    def forward(self, batch_feature_tensor_dict, mask):
        # Embedding层
        user_info_vec = self.user_info_embedding_layer(batch_feature_tensor_dict)
        ...
        user_history_not_pay_QOE_vec, user_history_not_pay_CHONGHE_vec, user_history_not_pay_FUFEI_vec = self.user_history_not_pay_embedding_layer(batch_feature_tensor_dict)
        target_vec = self.target_embedding_layer(batch_feature_tensor_dict)

        # History Attention层  pay & not pay
        HistoryDimScaling_Weight_Result_pay, se_user_history_pay_QOE_vec, se_user_history_pay_CHONGHE_vec, se_user_history_pay_FUFEI_vec = self.history_pay_attention_layer(user_history_pay_QOE_vec,  user_history_pay_CHONGHE_vec, user_history_pay_FUFEI_vec, mask)
        HistoryDimScaling_Weight_Result_not_pay, se_user_history_not_pay_QOE_vec, se_user_history_not_pay_CHONGHE_vec, se_user_history_not_pay_FUFEI_vec = self.history_not_pay_attention_layer(user_history_not_pay_QOE_vec, user_history_not_pay_CHONGHE_vec, user_history_not_pay_FUFEI_vec, mask)

        # Target with History Attention层
        target_history_pay_attention_weight, target_history_not_pay_attention_weight, target_history_pay_attention_vec, target_history_not_pay_attention_vec = self.target_history_attention_layer(
        ...
        target_history_not_pay_attention_vec = target_history_not_pay_attention_vec.view(batch_size, -1)
        ...
        user_info_vec = self.user_info_dim_change(user_info_vec)

        # (batch,200)->(batch,400)
        pay_vec = torch.cat(user_info_vec, target_history_pay_attention_vec, dim=1)
        not_pay_vec = torch.cat(user_info_vec, target_history_not_pay_attention_vec, dim=1)

        # MLP
        # (batch,400) ->MLP ->(batch，1) ->sigmoid -> (batch,1)
        pay_out_vec = self.pay_vec_MLP_layer(pay_vec)
        not_pay_out_vec = self.not_pay_vec_MLP_layer(not_pay_vec)
        # 使用softmax函数将logits转换为概率分布
        pay_score = F.softmax(pay_out_vec, dim=1)  # 在类别维度（dim=1）上应用softmax
        not_pay_score = F.softmax(not_pay_out_vec, dim=1)  # 在类别维度（dim=1）上应用softmax

        return pay_score, not_pay_score, HistoryDimScaling_Weight_Result_pay, HistoryDimScaling_Weight_Result_not_pay, target_history_pay_attention_weight, target_history_not_pay_attention_weight

# 损失函数
class LossFunction(nn.Module):
    def __init__(self, pay_weight=1.0, not_pay_weight=1.0):
        super(LossFunction, self).__init__()
        self.pay_weight = pay_weight
        self.not_pay_weight = not_pay_weight

    def forward(self, pred1, pred2, target1, target2):
        # pred1, pred2 是未经处理过的原值，target是0、1标签
        # 计算第一个任务的二元交叉熵损失
        loss1 = F.binary_cross_entropy_with_logits(pred1, target1, reduction='none')
        # 计算第二个任务的二元交叉熵损失
        loss2 = F.binary_cross_entropy_with_logits(pred2, target2, reduction='none')

        # 根据权重组合两个损失 根据权重组合两个损失，实现最小化 Loss1 - Loss2
        total_loss = self.pay_weight * loss1.mean() - self.not_pay_weight * loss2.mean()
...
def model_training(model,train_loader,val_loader, lossfunction,optimizer,EPOCH,device):
    ...
    for epoch in range(EPOCH):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            pay_score, not_pay_score, HistoryDimScaling_Weight_Result_pay, \
            HistoryDimScaling_Weight_Result_not_pay, target_history_pay_attention_weight, \
            target_history_not_pay_attention_weight = model(xxx, label, mask)  # 待修改

            pay_scores = pay_score[:, 0]  # (样本数，1)
            pay_scores = pay_scores.unsqueeze(-1)
            not_pay_scores = not_pay_score[:, 0]  # (样本数，1)
            not_pay_scores = not_pay_scores.unsqueeze(-1)
            loss, pay_loss, not_pay_loss = lossfunction(pay_scores, not_pay_scores, label, label)
            loss.backward()
            optimizer.step()
            # 损失
            total_pay_loss += pay_loss.item()
            total_not_pay_loss += not_pay_loss.item()
            total_loss += loss.item()
            # 平均损失
            average_pay_loss = total_pay_loss / len(train_loader)
            average_not_pay_loss = total_not_pay_loss / len(train_loader)
            average_loss = total_loss / len(train_loader)

            if (epoch + 1) % 5 == 0:
                print(
                    f"Epoch {epoch + 1},pay_Loss: {average_pay_loss},not_pay_Loss: {average_not_pay_loss},loss:{average_loss}")
                ...
                with torch.no_grad():  # 在评估模式下不计算梯度
                    total_loss_val = 0.0
                    total_auc_val = 0.0
                    total_pay_loss_val = 0.0
                    total_not_pay_loss_val = 0.0
                    for batch_val in val_loader:  # 假设你有一个名为 val_loader 的验证集数据加载器
                        ...
                        pay_score_val, not_pay_score_val, HistoryDimScaling_Weight_Result_pay_val, \
                            HistoryDimScaling_Weight_Result_not_pay_val, target_history_pay_attention_weight_val, \
                            target_history_not_pay_attention_weight_val = model(xxx_val, label_val, mask_val)  # 待修改

                        pay_scores_val = pay_score_val[:, 0]  # (样本数，1)
                        pay_scores_val = pay_scores_val.unsqueeze(-1)
                        not_pay_scores_val = not_pay_score_val[:, 0]  # (样本数，1)
                        not_pay_scores_val = not_pay_scores_val.unsqueeze(-1)
                        loss_val, pay_loss_val, not_pay_loss_val = lossfunction(pay_scores_val, not_pay_scores_val, label_val, label_val)
                        # 损失
                        total_pay_loss_val += pay_loss_val.item()
                        total_not_pay_loss_val += not_pay_loss_val.item()
                        total_loss_val += loss_val.item()
                        # 计算验证集上的AUC   待修改
                        correct_val = loss_val.sum().item()
                        total_auc_val += roc_auc_score(label_val, y_scores)
                        # 平均损失
                        average_pay_loss_val = total_pay_loss_val / len(val_loader)
                        average_not_pay_loss_val = total_not_pay_loss_val / len(val_loader)
                        average_loss_val = total_loss_val / len(val_loader)
                        average_auc_val = total_auc_val / len(val_loader)
                        print(f"Validation Loss: {average_loss_val},pay_Loss: {average_pay_loss_val},not_pay_Loss: {average_not_pay_loss_val},AUC: {average_auc_val}")
...
def test_model(model, test_loader):
    ...
    with torch.no_grad():  # 在评估模式下不计算梯度
        total_loss_test = 0.0
        total_pay_loss_test = 0.0
        total_not_pay_loss_test = 0.0
        total_auc_test = 0.0
        results = []  # 用于保存结果的列表
        for batch_test in test_loader:  # 假设你有一个名为 val_loader 的验证集数据加载器
            ...
            pay_score_test, not_pay_score_test, HistoryDimScaling_Weight_Result_pay_test, \
                HistoryDimScaling_Weight_Result_not_pay_test, target_history_pay_attention_weight_test, \
                target_history_not_pay_attention_weight_test = model(xxx, label_test, mask_test )  # 待修改


            pay_scores_test = pay_score_test[:, 0]  # (样本数，1)
            pay_scores_test = pay_scores_test.unsqueeze(-1)
            not_pay_scores_test = not_pay_score_test[:, 0]  # (样本数，1)
            not_pay_scores_test = not_pay_scores_test.unsqueeze(-1)
            loss_test, pay_loss_test, not_pay_loss_test = lossfunction(pay_score_test, not_pay_score_test, label_test,
                                                                       label_test)
            # 损失
            total_pay_loss_test += pay_loss_test.item()
            total_not_pay_loss_test += not_pay_loss_test.item()
            total_loss_test += loss_test.item()
            # 计算验证集上的AUC   待修改
            correct_test = loss_test.sum().item()
            total_auc_test += roc_auc_score(loss_test, y_scores)
            # 平均损失
            average_pay_loss_test = total_pay_loss_test / len(test_loader)
            average_not_pay_loss_test = total_not_pay_loss_test / len(test_loader)
            average_loss_test = total_loss_test / len(test_loader)
            average_auc_test = total_auc_test / len(test_loader)
            print(
                f"Test Loss: {average_loss_test},pay_Loss: {average_pay_loss_test},not_pay_Loss: {average_not_pay_loss_test},AUC: {average_auc_test}")
            return average_loss_test, average_pay_loss_test,average_not_pay_loss_test, average_auc_test
...
for i in range(5):
    ...
    # mask矩阵的字典
    train_batch_feature_tensor_history_mask_dict = generate_user_feature_alignment_tensor(train_data_tensor_hash_history_mask)
    val_batch_feature_tensor_history_mask_dict = generate_user_feature_alignment_tensor(val_data_tensor_hash_history_mask)
    test_batch_feature_tensor_history_mask_dict = generate_user_feature_alignment_tensor(test_data_tensor_hash_history_mask)
    ...
    model.to(device)
    # 进一步处理 列表转移到GPU
    for i in range(len(model.user_history_pay_embedding_layer.user_not_pay_history_QOE_discrete_embeddings)):
        model.user_history_pay_embedding_layer.user_not_pay_history_QOE_discrete_embeddings[i] = \
        model.user_history_pay_embedding_layer.user_not_pay_history_QOE_discrete_embeddings[i].to(device)
    for i in range(len(model.user_history_pay_embedding_layer.user_not_pay_history_CHONGHE_discrete_embeddings)):
        model.user_history_pay_embedding_layer.user_not_pay_history_CHONGHE_discrete_embeddings[i] = \
        model.user_history_pay_embedding_layer.user_not_pay_history_CHONGHE_discrete_embeddings[i].to(device)
    for i in range(len(model.user_history_pay_embedding_layer.user_not_pay_history_FUFEI_discrete_embeddings)):
        model.user_history_pay_embedding_layer.user_not_pay_history_FUFEI_discrete_embeddings[i] = \
        model.user_history_pay_embedding_layer.user_not_pay_history_FUFEI_discrete_embeddings[i].to(device)
    ...
    average_loss_test, average_pay_loss_test, average_not_pay_loss_test, average_auc_test = test_model(model, test_loader)
    # 测试的每个样本结果保存到csv
    # 将本次训练的结果添加到DataFrame中
    test_auc_df = test_auc_df.append(
        {'实验数': i + 1, '测试集总损失': average_loss_test, 'AUC': average_auc_test, 'pay损失': average_pay_loss_test,
         'not_pay损失': average_not_pay_loss_test}, ignore_index=True)
# 将结果保存到CSV文件中
