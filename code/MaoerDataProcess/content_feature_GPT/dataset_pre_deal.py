# -*- coding: utf-8 -*-
# 数据集预处理
# 从数据库中获取的数据包含空值及特殊字符，处理后生成prompt的json文件
import re
import pandas as pd
import json
...
def remove_special_chars(text):
    # 定义要删除的特殊字符的正则表达式模式
    pattern = r"<.*?>"  # 匹配以<开头，以>结尾的内容，即HTML标签
    # 使用sub函数替换匹配到的字符为""
...
input_file = '../content_feature_GPT/active_drama_intro_20240127_hasbeendeal_1.csv'
output_file = path+'content_feature_GPT_test_prompt.json'
...
for n in range(0, num_rows):
    ...
    content_file_content = remove_special_chars(content_file_content)  # 预处理


    system_pre = '你是一个能分辨文本内容类型的助手。'
    user_pre = '下面将输入一段文本，这是来源于一些长或短的故事简介。根据输入的文本内容，判断这段文本所指的故事可能所属的性向分类、时代分类、故事类型及标签。' \
               '具体的性向分类有：言情,纯爱,无CP；' \
    ...
    assistant_pre = '''。以以下json形式作为输出，输出格式为：result = {"标题": xx,"性向": yy,"时代": zz,"类型": vv,"标签": [ss,ff,gg,ee,...]}'''
