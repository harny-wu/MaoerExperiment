...
//		data.outDivide10DataFile();
		...
		if(args[1].equals("StaticReduce")) {
			System.out.println("||INEC2_threepart_turn静态约简程序开始，运行次数："+Integer.parseInt(args[2]));
			for(int k=0;k<Integer.parseInt(args[2]);k++) {
				...
				INEC2_threepart_turn method=new INEC2_threepart_turn();
				int num=9;  //num 加的次数		
				for(int i=0;i<=num;i++) {
					if(i!=0)
					...
					data.addProcessingResult("INEC2_threepart_turn",dataset,missingratio,"StaticReduce",i,end-start,result.reduceClone(),result.getReduce().size(),data.getU().size(),result.getSplitTimes(),result.TNECSize);	
		...
		if (args[1].equals("IncrementalReduce")) {
			...
			for (int k = 0; k < Integer.parseInt(args[2]); k++) {
				...
				INEC2_threepart_turn method2 = new INEC2_threepart_turn();
				INEC2_threepart_turn method3 = new INEC2_threepart_turn();
				start = System.currentTimeMillis();
				...
				data2.addProcessingResult("INEC2_threepart_turn", dataset, missingratio, "IncrementalReduce", 0,
						end - start, previousResult2.reduceClone(), previousResult2.getReduce().size(),
						data2.getU().size(), previousResult2.getSplitTimes(), previousResult2.TNECSize);
...
//					data2.getU().addAll(data2.getNewU());
					...
					data2.addProcessingResult("INEC2_threepart_turn", dataset, missingratio, "IncrementalReduce", i + 1,
							end - start, previousResult2.reduceClone(), previousResult2.getReduce().size(),
							data2.getU().size(), previousResult2.getSplitTimes(), previousResult2.TNECSize);
					data2.outPutProcessingofTimesResult(previousResult2.getSplitTimes());
