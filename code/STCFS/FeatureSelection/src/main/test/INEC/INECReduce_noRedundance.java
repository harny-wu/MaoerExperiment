...
//		data.outDivide10DataFile();
		...
		if(args[1].equals("StaticReduce")) {
			System.out.println("||INECReduce_noRedundance静态约简程序开始，运行次数："+Integer.parseInt(args[2]));
			for(int k=0;k<Integer.parseInt(args[2]);k++) {
				...
				INEC_noRedundance method=new INEC_noRedundance();
				int num=9;  //num 加的次数		
				for(int i=0;i<=num;i++) {
					if(i!=0)
					...
					data.addProcessingResult("INECReduce_noRedundance",dataset,missingratio,"StaticReduce",i,end-start,result.reduceClone(),result.getReduce().size(),data.getU().size(),result.getSplitTimes(),result.TNECSize);	
		...
		if (args[1].equals("IncrementalReduce")) {
			System.out.println("||INEC_noRedundance动态约简程序开始，运行次数：" + Integer.parseInt(args[2]));
			for (int k = 0; k < Integer.parseInt(args[2]); k++) {
				System.out.println("||当前运行次数：" + (k + 1));
				...
				INEC_noRedundance method2 = new INEC_noRedundance();
				INEC_noRedundance method3 = new INEC_noRedundance();
				start = System.currentTimeMillis();
				...
				data2.addProcessingResult("INEC_noRedundance", dataset, missingratio, "IncrementalReduce", 0,
...
//					data2.getU().addAll(data2.getNewU());
					...
					data2.addProcessingResult("INEC_noRedundance", dataset, missingratio, "IncrementalReduce", i + 1,
							end - start, previousResult2.reduceClone(), previousResult2.getReduce().size(),
							data2.getU().size(), previousResult2.getSplitTimes(), previousResult2.TNECSize);
					data2.outPutProcessingofTimesResult(previousResult2.getSplitTimes());
