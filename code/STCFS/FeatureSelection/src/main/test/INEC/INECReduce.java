...
import main.java.INEC.algorithm.INEC;
...
//		data.outDivide10DataFile();
		...
		if(args[1].equals("StaticReduce")) {
			System.out.println("||INEC静态约简程序开始，运行次数："+Integer.parseInt(args[2]));
			for(int k=0;k<Integer.parseInt(args[2]);k++) {
				...
				for(int i=0;i<=num;i++) {
					...
					data.addProcessingResult("INEC",dataset,missingratio,"StaticReduce",i,end-start,result.reduceClone(),result.getReduce().size(),data.getU().size(),result.getSplitTimes(),result.TNECSize);	
		...
		if (args[1].equals("IncrementalReduce")) {
			System.out.println("||INEC动态约简程序开始，运行次数：" + Integer.parseInt(args[2]));
			for (int k = 0; k < Integer.parseInt(args[2]); k++) {
				System.out.println("||当前运行次数：" + (k + 1));
				...
				INEC method2 = new INEC();
				INEC method3 = new INEC();
				start = System.currentTimeMillis();
				...
				data2.addProcessingResult("INEC", dataset, missingratio, "IncrementalReduce", 0, end - start,
...
//					data2.getU().addAll(data2.getNewU());
					...
					data2.addProcessingResult("INEC", dataset, missingratio, "IncrementalReduce", i + 1, end - start,
							previousResult2.reduceClone(), previousResult2.getReduce().size(), data2.getU().size(),
							previousResult2.getSplitTimes(), previousResult2.TNECSize);
					data2.outPutProcessingofTimesResult(previousResult2.getSplitTimes());
