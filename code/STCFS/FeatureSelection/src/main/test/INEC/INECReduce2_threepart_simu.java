...
import main.java.INEC.algorithm.INEC2_threepart_simu;
...
//约简
public class INECReduce2_threepart_simu {
    static {
    ...
    public static void main(String[] args) {
        ...
        // //运行一个文件
        // String
        // filePathnew="C:\\Users\\凌Y\\Desktop\\数据挖掘学习\\测试数据\\测试数据_INEC\\5%_MissingValue\\car\\car.csv";
        // //运行一个文件夹
...
//		data.outDivide10DataFile();
        ...
        if (args[1].equals("StaticReduce")) {
            System.out.println("||INEC2_threepart_simu静态约简程序开始，运行次数：" + Integer.parseInt(args[2]));
            for (int k = 0; k < Integer.parseInt(args[2]); k++) {
                ...
                INEC2_threepart_simu method = new INEC2_threepart_simu();
                int num = 9;  //num 加的次数
                for (int i = 0; i <= num; i++) {
                    if (i != 0)
                    ...
                    testAllU.addAll(data.getU());
                    testAllU.addAll(data.getNewU());
                    ...
                    data.addProcessingResult("INEC2_threepart_simu", dataset, missingratio, "StaticReduce", i, end - start, result.reduceClone(), result.getReduce().size(), data.getU().size(), result.getSplitTimes(), result.TNECSize);
        ...
        if (args[1].equals("IncrementalReduce")) {
            ...
            for (int k = 0; k < Integer.parseInt(args[2]); k++) {
                ...
                INEC2_threepart_simu method2 = new INEC2_threepart_simu();
                INEC2_threepart_simu method3 = new INEC2_threepart_simu();
                start = System.currentTimeMillis();
                ...
                size[0] = previousResult2.getInitial_te().getComplete().size() + previousResult2.getInitial_te().getIncomplete().size();
                int TESize = 0;
                for (toleranceClass tc : previousResult2.getInitial_te().getComplete().values())
                    TESize += tc.getTE().size();
                for (toleranceClass tc : previousResult2.getInitial_te().getIncomplete().values())
                    TESize += tc.getTE().size();
                size[1] = TESize;

                data2.addProcessingResult("INEC2_threepart_simu", dataset, missingratio, "IncrementalReduce", 0,
                        end - start, previousResult2.reduceClone(), previousResult2.getReduce().size(),
                        data2.getU().size(), previousResult2.getSplitTimes(), size, previousResult2.TNECSize);
...
//					data2.getU().addAll(data2.getNewU());
                    ...
                    size2[0] = previousResult2.getInitial_te().getComplete().size() + previousResult2.getInitial_te().getIncomplete().size();
                    int TESize2 = 0;
                    for (toleranceClass tc : previousResult2.getInitial_te().getComplete().values())
                        TESize2 += tc.getTE().size();
                    for (toleranceClass tc : previousResult2.getInitial_te().getIncomplete().values())
                        TESize2 += tc.getTE().size();
                    size2[1] = TESize2;

                    data2.addProcessingResult("INEC2_threepart_simu", dataset, missingratio, "IncrementalReduce", i + 1,
                            end - start, previousResult2.reduceClone(), previousResult2.getReduce().size(),
                            data2.getU().size(), previousResult2.getSplitTimes(), size2, previousResult2.TNECSize);
                    //data2.outPutProcessingofTimesResult(previousResult2.getSplitTimes());
