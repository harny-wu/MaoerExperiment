...
public class FSMVReduce {
	...
	public static void main(String[] args) {
		...
		if(args[1].equals("StaticReduce")) {
			System.out.println("||FSMV静态约简程序开始，运行次数："+Integer.parseInt(args[2]));
			for(int k=0;k<Integer.parseInt(args[2]);k++) {
				...
				for(int i=0;i<=num;i++) {
					...
					StaticReduceResult result=method.staticReduce_CFSV(data.getU(), data.getCName().length);
					...
					data.addProcessingResult("FSMV",dataset,missingratio,"StaticReduce",i,end-start,result.reduceClone(),result.getReduce().size(),data.getU().size(),result.getTimes());	
		...
		if (args[1].equals("IncrementalReduce")) {
			System.out.println("||FSMV动态约简程序开始，运行次数：" + Integer.parseInt(args[2]));
			for (int k = 0; k < Integer.parseInt(args[2]); k++) {
				System.out.println("||当前运行次数：" + (k + 1));
				...
				FSMV method2 = new FSMV();
				FSMV method3 = new FSMV();
				start = System.currentTimeMillis();
				StaticReduceResult previousResult2 = method2.staticReduce_CFSV(data2.getU(), data2.getCName().length);
				end = System.currentTimeMillis();
				times = new Formatter().format("%.2f", ((double) (end - start) / 1000)).toString();
				System.out.println("||程序运行时间：" + (end - start) + "ms,即" + times + "s");
				System.out.println(
						"||约简Reduce：" + previousResult2.getReduce() + ",约简数量：" + previousResult2.getReduce().size());
				data2.addProcessingResult("FSMV", dataset, missingratio, "IncrementalReduce", 0, end - start,
...
//					times=new Formatter().format("%.2f", ((double)(end-start)/1000)).toString();
//					System.out.println("程序运行时间："+(end-start)+"ms,即"+times+"s");
//					System.out.println("实际约简："+previousResult3.getReduce()+"约简数量："+previousResult3.getReduce().size()+" usize:"+testAllU.size());
//					data2.getU().addAll(data2.getNewU());

					System.out.println("---------U+newU的第" + i + "次增量约简结果-----------");
					start = System.currentTimeMillis();
					previousResult2 = method2.FSMVReduce(data2.getU(), data2.getNewU(), data2.getCName().length,
							previousResult2);
					end = System.currentTimeMillis();
					times = new Formatter().format("%.2f", ((double) (end - start) / 1000)).toString();
					System.out.println("||程序运行时间：" + (end - start) + "ms,即" + times + "s");
					...
					data2.addProcessingResult("FSMV", dataset, missingratio, "IncrementalReduce", i + 1, end - start,
							previousResult2.reduceClone(), previousResult2.getReduce().size(), data2.getU().size(),
							previousResult2.getTimes());
					data2.outPutProcessingofTimesResult(previousResult2.getTimes());
...
//		data.setU(u);
//		FSMV method=new FSMV();
//		StaticReduceResult previousResult=method.staticReduce_CFSV(data.getU(), data.getCName().length);
...
//		}		
//		data.setNewU(u);
//		previousResult=method.FSMVReduce(data.getU(), data.getNewU(), data.getCName().length, previousResult);
